...
module.exports = async(vm, runArgs, callback) => {
  ...
  } else if ((result.vm && result.vm.exceptionError) || (result.execResult && result.execResult.exceptionError)) {
    return callback(new Error(`execution error: ${result.vm.exceptionError.error}`));
  } else if (steps.done()) {
