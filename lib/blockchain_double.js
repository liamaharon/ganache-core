...
var VM = require("/home/david/work/ethereumjs-vm/dist/");
// var VM = require("ethereumjs-vm").default || require("ethereumjs-vm");
...
BlockchainDouble.prototype.processCall = function(tx, blockNumber, callback) {
  this.readyCall(tx, true, blockNumber, async(err, vm, runArgs) => {
    ...
    const runTx = vm.runTx.length === 1 ? vm.runTx.bind(vm) : promisify(vm.runTx.bind(vm));
    const result = await runTx(runArgs).catch((vmerr) => ({ vmerr }));
...
BlockchainDouble.prototype.processBlock = async function(vm, block, commit, callback) {
  ...
  const runBlock = vm.runBlock.length === 1 ? vm.runBlock.bind(vm) : promisify(vm.runBlock.bind(vm));
  const results = await runBlock({
    block: block,
    generate: true,
  ...
  for (var v = 0; v < results.receipts.length; v++) {
    ...
    const vm = result.vm || result.execResult;
    if (vm.exceptionError === undefined) {
